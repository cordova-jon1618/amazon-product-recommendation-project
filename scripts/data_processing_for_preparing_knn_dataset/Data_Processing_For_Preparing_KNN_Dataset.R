...
printer_categories <- c("furniture","office-products")

################################################################
# Data Integration (combining all of the scraped data)
###############################################################
########## Categories (1) books (including audiobooks and ebooks)
########## Categories (2) office chairs 
########## Categories (3) printers
...
# file_path <- ".csv"
# printer_data <- read.csv(file_path)
# 
# file_path <- ".csv"
# printer_data_two <- read.csv(file_path)
# 
# allowed_categories <- append(allowed_categories, office_categories)
# loaded_printer_data <- bind_rows(office_data, office_data_two)
# 
# ## Combine all of the data ##
# data <- bind_rows(loaded_book_data, loaded_office_data)

### All Category (for TRAINING DATA only) ##########################################################

# Specify the file path to your CSV file and read it into data

file_path <- "./Dataset_Collected_Raw.csv"
...
# Get the size of the processed data
dim(processed_data)  # This returns a vector with the number of rows and columns
nrow(processed_data) # Just the number of rows
ncol(processed_data) # Just the number of columns

# Print out the size
cat("The processed data has", nrow(processed_data), "rows and", ncol(processed_data), "columns.\n")

# Do the same for the incomplete data
head(processed_incomplete_data)

# Get the size of the incomplete data
dim(processed_incomplete_data)
nrow(processed_incomplete_data)
ncol(processed_incomplete_data)

# Print out the size
cat("The processed incomplete data has", nrow(processed_incomplete_data), "rows and", ncol(processed_incomplete_data), "columns.\n")
...
non_na_reviews <- processed_data %>%
  filter(!is.na(ReviewBody)) %>%
  mutate(ReviewBody = iconv(ReviewBody, from = "UTF-8", to = "UTF-8", sub = ""))  # Clean encoding issues
...
# Ensure that ReviewBody entries are in UTF-8 encoding #########
# Replace non-UTF-8 characters with empty strings if conversion fails
# non_na_reviews$ReviewBody <- iconv(non_na_reviews$ReviewBody, from = "UTF-8", to = "UTF-8", sub = "")
...
write.csv(readable_data, "Complete_Readable_Results.csv", row.names = FALSE)


##########################################################
# FINISHES WITH THE DATA CLEANING AND DATA PREPARATION
##########################################################
